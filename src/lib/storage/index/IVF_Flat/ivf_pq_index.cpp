...
IVFPQIndex::IVFPQIndex(const std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>& chunks_to_index,
  ...
  Assert(!chunks_to_index.empty(), "IVFPQIndex requires chunks_to_index not to be empty.");
  ...
  if (get_item = parameters.find("nbits"), get_item == parameters.end())
    _nbits_per_idx = DEFAULT_NBITS;
  else
  ...
  std::cout << "IVFPQIndex time(" << per_table_index_timer.lap_formatted() << ")" << std::endl;
}

bool IVFPQIndex::is_index_for(const ColumnID column_id) const {
  return column_id == _column_id;
}

ColumnID IVFPQIndex::get_indexed_column_id() const {
  return _column_id;
}

void IVFPQIndex::train_and_insert(const std::vector<std::pair<ChunkID, std::shared_ptr<Chunk>>>& chunks_to_index) {
...
void IVFPQIndex::similar_k(const float* query, int64_t* I, float* D, int k) {
  std::cout << "We do not recommend you to search for single queries." << std::endl;
  _index->search(1, query, k, D, I);
}

void IVFPQIndex::range_similar_k(size_t n, const float* query, int64_t* I, float* D, int k) {
  // std::cout << "IVFPQIndex::range_similar_k" << std::endl;
  auto per_table_index_timer = Timer{};
