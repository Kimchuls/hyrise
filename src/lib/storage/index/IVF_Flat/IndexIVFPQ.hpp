...
#ifndef VINDEX_INDEX_IVFPQ_H
#define VINDEX_INDEX_IVFPQ_H
...
VINDEX_API extern size_t precomputed_table_max_bytes;
...
struct IndexIVFPQ : IndexIVF {
    ...
    void train_encoder(int64_t n, const float* x, const int64_t* assign) override;

    int64_t train_encoder_num_vectors() const override;
     ...
     */
    size_t find_duplicates(int64_t* ids, size_t* lims) const;

    // map a vector to a binary code knowning the index
    void encode(int64_t key, const float* x, uint8_t* code) const;
...
VINDEX_API extern int index_ivfpq_add_core_o_bs;
...
VINDEX_API extern IndexIVFPQStats indexIVFPQ_stats;
