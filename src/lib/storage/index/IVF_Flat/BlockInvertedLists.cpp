...
size_t BlockInvertedLists::add_entries(
    ...
    VINDEX_THROW_IF_NOT(list_no < nlist);
    ...
    if (o % block_size == 0) {
        ...
        VINDEX_THROW_IF_NOT_MSG(packer, "missing code packer");
...
const int64_t* BlockInvertedLists::get_ids(size_t list_no) const {
    assert(list_no < nlist);
...
void BlockInvertedLists::update_entries(
        ...
        const int64_t*,
        const uint8_t*) {
    VINDEX_THROW_MSG("not impemented");
    /*
    assert (list_no < nlist);
