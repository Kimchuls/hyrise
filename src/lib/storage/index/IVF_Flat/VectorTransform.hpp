...
#ifndef VINDEX_VECTOR_TRANSFORM_H
#define VINDEX_VECTOR_TRANSFORM_H
...
struct VectorTransform {
     ...
     */
    float* apply(int64_t n, const float* x) const;
     ...
     */
    virtual void apply_noalloc(int64_t n, const float* x, float* xt) const = 0;

    /// reverse transformation. May not be implemented or may return
    /// approximate result
    virtual void reverse_transform(int64_t n, const float* xt, float* x) const;
...
struct LinearTransform : VectorTransform {
    ...
    void apply_noalloc(int64_t n, const float* x, float* xt) const override;

    /// compute x = A^T * (x - b)
    /// is reverse transform if A has orthonormal lines
    void transform_transpose(int64_t n, const float* y, float* x) const;

    /// works only if is_orthonormal
    void reverse_transform(int64_t n, const float* xt, float* x) const override;
