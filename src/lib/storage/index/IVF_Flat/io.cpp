...
namespace vindex
{
  ...
  int IOReader::fileno()
  {
    VINDEX_THROW_MSG("IOReader does not support memory mapping");
  }

  int IOWriter::fileno()
  {
    VINDEX_THROW_MSG("IOWriter does not support memory mapping");
  }
  ...
  {
    VINDEX_THROW_IF_NOT(4 == strlen(sx));
  ...
  {
    VINDEX_THROW_IF_NOT(sx.length() == 4);
