...
namespace vindex
{
    ...
    void RangeSearchResult::do_allocation()
    {
        ...
        // simulatenously
        VINDEX_THROW_IF_NOT(labels == nullptr && distances == nullptr);
        size_t ofs = 0;
        for (int i = 0; i < nq; i++)
        {
            size_t n = lims[i];
    ...
    void BufferList::add(int64_t id, float dis)
    {
        if (wp == buffer_size)
        { // need new buffer
            append_buffer();
    ...
    {
        Buffer buf = {new int64_t[buffer_size], new float[buffer_size]};
    ...
    void RangeQueryResult::add(float dis, int64_t id)
    {
        nres++;
        pres->add(id, dis);
    }
    ...
    /// begin a new result
    RangeQueryResult &RangeSearchPartialResult::new_result(int64_t qno)
    {
