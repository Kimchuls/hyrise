...
#ifdef VINDEX_MAIN_LIB
#define VINDEX_API __declspec(dllexport)
#else // _VINDEX_MAIN_LIB
#define VINDEX_API __declspec(dllimport)
#endif // VINDEX_MAIN_LIB
...
#define VINDEX_DEPRECATED(msg) __attribute__((deprecated(msg)))
#else
#define VINDEX_DEPRECATED(msg)
#endif // GCC or Clang

// Localized enablement of imprecise floating point operations
// You need to use all 3 macros to cover all compilers.
#if defined(_MSC_VER)
#define VINDEX_PRAGMA_IMPRECISE_LOOP
#define VINDEX_PRAGMA_IMPRECISE_FUNCTION_BEGIN \
    __pragma(float_control(precise, off, push))
#define VINDEX_PRAGMA_IMPRECISE_FUNCTION_END __pragma(float_control(pop))
...
#define VINDEX_PRAGMA_IMPRECISE_FUNCTION_END _Pragma("float_control(pop)")
#else
#define VINDEX_PRAGMA_IMPRECISE_FUNCTION_BEGIN
#define VINDEX_PRAGMA_IMPRECISE_FUNCTION_END
#endif
